...
def search(request, query_id=None):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # existing_query = check_existing_query(query_text, user)

            # if existing_query:
            #     print("existing_queryexisting_queryexisting_query")
            #     # Calculate the time elapsed since the last search
            #     time_elapsed = datetime.now() - existing_query.created_at
                
            #     if time_elapsed >= timedelta(minutes=15):
            #         print("time_elapsedtime_elapsedtime_elapsed")
            ...
            #         last_timestamp = existing_query.created_at
            #         search_results = perform_search(query_text, last_timestamp)
            ...
            #             search_result = SearchResult(query=existing_query, title=result['title'], description=result['description'], source_name=result['source_name'], source_url=result['source_url'], published_at=result['published_at'])
            #             search_result.save()
            ...
            #         return render(request, 'searchapp/search_results.html', {'search_results': search_results, 'search_query': existing_query})
    ...
    if request.method == 'POST':
        ...
        if existing_query:
            ...
            if time_elapsed.total_seconds() >= 900:  # 15 minutes = 900 seconds
                # If it's been 15 minutes or more, perform a new search
                # Perform the actual search using an API (code for this part is explained later)
                search_results = perform_search(query_text, existing_query.created_at)
